...
@php
    ...
    $isPostOnly = (bool) $product->post_only;

    $waNumber  = preg_replace('/\D/', '', core()->getConfigData('catalog.products.whatsapp.number') ?? '');
    $waText    = core()->getConfigData('catalog.products.whatsapp.inquiry_message')
                 ?: trans('custom-theme::app.products.view.whatsapp-inquiry', ['name' => $product->name, 'url' => url()->current()]);
    $waMessage = urlencode(str_replace([':name', ':url'], [$product->name, url()->current()], $waText));
                            ...
                            <div class="relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                ...
                                @if ($waNumber)
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener"
                                        class="mt-5 flex w-full max-w-[470px] items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-3 text-base font-medium text-white transition hover:bg-[#1ebe57] max-md:py-3 max-sm:mt-3 max-sm:rounded-lg max-sm:py-1.5"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current" viewBox="0 0 24 24">
                                            ...
                                            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.553 4.117 1.522 5.848L0 24l6.335-1.502A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 0 1-5.002-1.366l-.36-.213-3.757.89.952-3.648-.234-.374A9.818 9.818 0 1 1 12 21.818z"/>
                                        </svg>
                                        @lang('custom-theme::app.products.view.whatsapp-inquiry-button')
                                    </a>
                                @endif
