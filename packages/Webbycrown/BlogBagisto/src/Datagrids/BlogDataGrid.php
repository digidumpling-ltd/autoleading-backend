...
class BlogDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        ...
        $queryBuilder = DB::table('blogs')
            ->leftJoin('blog_translations', function ($join) use ($currentLocale) {
                $join->on('blogs.id', '=', 'blog_translations.blog_id')
                    ->where('blog_translations.locale', '=', $currentLocale);
            });
    ...
    public function prepareColumns()
    {
        ...
        $this->addColumn([
            'index' => 'src',
            'label' => trans('blog::app.datagrid.image'),
            'type' => 'string',
            'searchable' => false,
            'sortable' => false,
            'filterable' => false,
            'closure' => function ($row) {
                if ($row->src) {
                    return '<img src="' . e(Storage::url($row->src)) . '" class="h-10 w-10 rounded object-cover" />';
                }

                return '<div class="h-10 w-10 rounded border-2 border-dashed border-gray-300 dark:border-gray-600"></div>';
            },
        ]);
        ...
        $this->addColumn([
            ...
            'closure' => function ($value) {
                ...
                if (is_array($categories_ids) && ! empty($categories_ids) && count($categories_ids) > 0) {
                    ...
                    $categories_names = (! empty($categories) && count($categories) > 0) ? $categories->pluck('name')->toarray() : [];
        ...
        $this->addColumn([
            ...
            'closure' => function ($value) {
                ...
                if (is_array($tags_ids) && ! empty($tags_ids) && count($tags_ids) > 0) {
                    ...
                    $tags_names = (! empty($tag_deatils) && count($tag_deatils) > 0) ? $tag_deatils->pluck('name')->toarray() : [];
