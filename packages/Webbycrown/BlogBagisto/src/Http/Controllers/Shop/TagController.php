...
class TagController extends Controller
{
    ...
    public function index($tag_slug)
    {
        ...
        $tag = Tag::whereHas('translations', function ($q) use ($tag_slug, $locale) {
            $q->where('slug', $tag_slug)->where('locale', $locale);
        })->firstOr(function () use ($tag_slug) {
            return Tag::whereHas('translations', function ($q) use ($tag_slug) {
