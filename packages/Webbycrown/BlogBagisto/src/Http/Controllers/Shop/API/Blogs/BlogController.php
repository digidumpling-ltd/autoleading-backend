    ...
    public function list()
    {
        try{
            ...
            if ( array_key_exists( 'slug', $req_data ) ) {
                ...
                $blogs = Blog::whereTranslation('slug', $blog_slug)
                ->where('published_at', '<=', Carbon::now()->format('Y-m-d'))
                ->where('status', 1)
                ->orderBy('id', 'DESC')
                ->first();
            } else {
                $blogs = Blog::where('published_at', '<=', Carbon::now()->format('Y-m-d'))->where('status', 1);
                ...
                if ( isset( $search ) && !empty( $search ) && !is_null( $search ) ) {
                    $blogs = $blogs->whereTranslationLike('name', '%' . $search . '%');
                }
